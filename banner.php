...
move_uploaded_file($app_tmp, $p1 .$app_name) ;
 ...
 $p1= "imgaes/" ;
  ...
  $addapp= $database->prepare("INSERT INTO `banner` ( img ) VALUES (:img );");
...
$database->prepare("DELETE FROM banner WHERE id=?")->execute([$id]);
...
while ($result=$stmt->fetch()){
    $img=$result['img'];
			...
			echo '
					<tr>
					  <th scope="row">'.$num++.'</th>
					  <td><img width="50" height="50" src="'.$p.$img.'"></img></td>
